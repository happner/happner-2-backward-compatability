...
describe(path.basename(__filename), function (done) {
  ...
  it('contains the mesh name and version', function () {
    ...
    var happner2Path = path.resolve(__dirname,['..', 'node_modules', 'happner-2'].join(path.sep));

    expect(testClient.info.version).to.be(require(happner2Path  + '/package.json').version);
